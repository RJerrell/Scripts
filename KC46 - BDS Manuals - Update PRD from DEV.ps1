...
function Update-ProductionCSDB
{
    ...
    [string[]] $PubList   = @("ABDR","ACS","ASIP","LOAPS","SIMR","SPCC","WUC")
    foreach($Pub in $PubList)
    {
        ...
        $files = gci -Path $sdmPath -Filter *DMC*.XML | Sort-Object -Descending
        $pmcfiles = gci -Path $sdmPath -Filter *PMC*.XML | Sort-Object -Descending | Select-Object -First 1
        ...
        foreach ($file in $files)
        {
            ...
            if(! $processedFiles.Contains($basePart))
            {
                ...
                $basefiles = gci -Path $sdmPath -Filter "$basePart`*.xml" | Sort-Object -Descending | Select -First 1
        ...
        {md $tIllPath1}
        $allGraphics = gci -Path "$sIllPath\*" -Include *.cgm,*.png
