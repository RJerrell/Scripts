...
Purpose: Create an Excel spreadsheet for distribution internally to Boeing Technical Publications leaders
...
$env:PSModulePath = "C:\Program Files (x86)\PowerShell Community Extensions\Pscx3\;C:\Windows\system32\WindowsPowerShell\v1.0\Modules\;f:\KC46 Staging\Scripts\Modules;"
...
# Previous Release Data path
$prevReleasePath = "F:\10.3\Manuals\AMM\S1000D\SDLLIVE"

# Current Release Data path
$currentReleasePath = "F:\KC46 Staging\Production\Manuals\AMM\S1000D\SDLLIVE"
...
$reportName = "BOEING Verification - Releases 10 ~ 11 " + $rptTimeStamp + ".xlsx"
$reportName
...
#[string[]] $PubList   = @("SWPM")
$getAllHistory = $false # Set to true to get all the history back to Release 5
$ignored = @()
...
foreach ($Pub in $PubList)
{
    ...
    {       
        ...
        Copy-Item -Path $pathToTheGOOOOPile -Destination F:\ -Force

        $pathToTheGOOOOPile = "F:\DMCertVerWorksheet.xlsx"
    ...
    $currentReleaseNewPath = $currentReleasePath.Replace("AMM",$Pub)
    $pms = gci -Path "$currentReleaseNewPath\PMC*.XML" -File
    ...
    foreach ($dmRef in $dmRefs)
    {
        ...
        if($book -ne $Pub)
        {
            $ignored += $fileName + " ignored because that DMC is not part of the $Pub"
            
        }
        else
        {
            $dmPath = $currentReleasePath.Replace("AMM",$book)
            ...
            if($fileOnDisk.Count -gt 0)
            {
                ...
                $currentIssueNum = $parserDM.IssueInfo.issueNumber
                $infoName =  $parserDM.InfoName
                $techName =  $parserDM.TechName

                if(($currentIssueNum -eq "001") -and ($parserDM.IdentAndStatusSection.dmStatus.issueType -eq "new") )
                {
            ...
            # Previous Release
            $prevReleaseIssue = ""
            $prevReleaseNewPath = $prevReleasePath.Replace("AMM",$Pub)
            $fileOnDisk = gci -Path "$prevReleaseNewPath\$fileName`*.xml"| Sort-Object -Descending | Select-Object -First 1
            if($fileOnDisk.Count -gt 0)
            {
                $parserDM.ParseDM($fileOnDisk[0].FullName)
                $prevReleaseIssue = $parserDM.IssueInfo.issueNumber
            }

            $currentIssue = $currentReleaseIssue 
    
            $issuedUp = $false
            if($prevReleaseIssue.Length -gt 0)
            {
                if([int]$currentIssueNum -gt [int]$prevReleaseIssue)
                {
                    $issuedUp = $true
                }
            }
            ...
            $masterList += New-Object -TypeName PSObject -Property @{
                ...
                Rel_7 = $rel7Issue;
                Rel_8 = $rel8Issue;
                Rel_10 = $prevReleaseIssue;
                Rel_11 = $currentIssueNum;
                BOE_CertificationType = $boeCertType;
                USAF_VerificationType = "";
                RFU_List = $rfuEntry
                
            } | Select Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_5,Rel_6,Rel_7,Rel_8,Rel_9,Rel_10,Rel_11,BOE_CertificationType,USAF_VerificationType,RFU_List
            }
            else
            {
                    $masterList += New-Object -TypeName PSObject -Property @{
                ...
                } | Select Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_10,Rel_11,BOE_CertificationType,USAF_VerificationType,RFU_List
...
if($getAllHistory)
{
    $masterList | Export-XLSX -Path "$exportFolder\$reportName" -Header  Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_5,Rel_6,Rel_7,Rel_8,Rel_9,Rel_10,Rel_11,BOE_CertificationType,USAF_VerificationType,RFU_List -WorksheetName "Verifications"
}
else
{
    $masterList | Export-XLSX -Path "$exportFolder\$reportName" -Header  Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_10,Rel_11,BOE_CertificationType,USAF_VerificationType,RFU_List -WorksheetName "Verifications"
}
