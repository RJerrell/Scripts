...
$rel10Path = "D:\Shared\IDE cd sets\Releases\Aug 2018 - Release 10.2\CSDB\DVD\AMM\S1000D\SDLLIVE"
...
$reportName = "BOEING Verification - Releases 5 thru 10.2 - " + $rptTimeStamp + ".xlsx"
$reportName
...
foreach ($Pub in $PubList)
{
    ...
    $pms = gci -Path "$rel10NewPath\PMC*.XML" -File
    ...
    foreach ($dmRef in $dmRefs)
    {
        ...
        {
            $dmPath = $rel10Path.Replace("AMM",$book)
            ...
            else
            {
                ...
                } | Select Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_9,Rel_10,BOE_CertificationType,USAF_VerificationType,RFU_List
...
{
    $masterList | Export-XLSX -Path "$exportFolder\$reportName" -Header  Manual,VerListValue,Type,PBTask,DMC,TechName,InfoName,Rel_9,Rel_10,BOE_CertificationType,USAF_VerificationType,RFU_List -WorksheetName "Verifications"
}
