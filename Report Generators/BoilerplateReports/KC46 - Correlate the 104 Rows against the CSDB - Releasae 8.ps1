...
foreach ($wsRow in $wsRows)
{
    ...
    {    
        ...
        if((! $certComp.Date.ToString().Contains("1899"))-and $pbTask.Length -gt 0)
        {
...
foreach ($dmRef in $dmRefs)
    ...
    } | Select Type,PBTask,DMC,TechName,InfoName,Rel_5,Rel_6,Rel_7,Rel_8,BOE_CertificationType,VerificationType,RFU_List
    $ctr ++
    "Processing $ctr"
    }
...
"Out of " + $wsRows.Count + " total rows in the DMCertVerWorksheet.xlsx , " + $verCompListArray.Count + " had recognizable PB/Task values and had a discernable CERT Date and should be recognized as CERT'd"
...
$masterList | Export-XLSX -Path "$exportFolder\$reportName" -Header  Type,PBTask,DMC,TechName,InfoName,Rel_5,Rel_6,Rel_7,Rel_8,BOE_CertificationType,VerificationType,RFU_List -WorksheetName "Verifications"
