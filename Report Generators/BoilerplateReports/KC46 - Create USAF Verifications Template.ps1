...
#$rel11Path = "D:\Shared\IDE cd sets\Releases\May 2018 - Release 11\CSDB\DVD\AMM\S1000D\SDLLIVE"
#$rel12Path = "D:\Shared\IDE cd sets\Releases\May 2018 - Release 12\CSDB\DVD\AMM\S1000D\SDLLIVE"
...
$reportName = "BOEING Verification - Releases 5 thru 10 - " + $rptTimeStamp + ".xlsx"
$reportName
...
$pms = gci -Path "$rel10Path\PMC*.XML" -File
...
foreach ($dmRef in $dmRefs)
{
    ...
    $fileOnDisk = gci -Path "$rel10Path\$fileName`*.xml"| Sort-Object -Descending | Select-Object -First 1
    ...
    # Release 9
    $rel8Issue = ""
    $fileOnDisk9 = gci -Path "$rel9Path\$fileName`*.xml"| Sort-Object -Descending | Select-Object -First 1
    if($fileOnDisk8.Count -gt 0)
    {
        $parser.ParseDM($fileOnDisk9[0].FullName)
