...
foreach ($pub in $PubList)
{
    ...
    foreach ($file in $files)
    {
        ...
        $obj = [pscustomobject][ordered]@{Total_COC_Entries="$Total_COC_Entries";CAS_COC_DetailsCount="$CAS_COC_DetailsCount";BDS_COC_Details="$BDS_COC_Details";Manual=$pub;TechName=$techName;InfoName=$infoName;DMC=$FSN;issueNumber=$issueNumber;secondVerification="";Type=$dmType;}
        $objs += $obj
        $obj=$null
    }     
}
...
# $proc , $desc  = $fullDataSet.Where({$_.Type -eq "procedure" -and ($_.Manual -ne "LOAPS" ) },'Split')
