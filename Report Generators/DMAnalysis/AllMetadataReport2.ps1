...
foreach ($pub in $PubList)
   {       
       $pub
       $files = gci -Path "$source_BaseLocation\$pub\S1000D\SDLLIVE\DMC*.XML"
       $fCounter = 0
       foreach ($file in $files)
       {
            ...
            # Reference data module processing
            if($dmcodeColl.Count -gt 0)
            {
                [int32] $ctr =  0
                foreach($dmCode in $dmcodeColl)
                {
                    $dmcFilename = Create-DMCFileNameFromDMCode -dmCode $dmCode
                    if($dmcFilename -eq "DMC-1KC46-A-27-61-0000-04A0A-270A-A")
                    {
                        "stop here"
                    }
                    $path = "$source_BaseLocation\$pub\S1000D\SDLLIVE\$dmcFilename.xml"

                    $fi = gci -Path $path

                    $refDM = [xml] ( Get-Content -Path $path )

                    if($refDM.DocumentElement.content.procedure.preliminaryRqmts.ChildNodes.Count -gt 1 )
                    {
                        $refDM.DocumentElement.content.ChildNodes[1].Name
                    }
                    else
                    {
                        break
